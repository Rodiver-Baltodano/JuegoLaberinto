...
def cargarAudio(nombre):
    ruta = os.path.join('imagenes', nombre)
    return ruta

def play(audio):
    pygame.mixer.init()
    pygame.mixer.music.load(cargarAudio(audio))
...
def UpVolume():
    global volume
    volume = min(1.0, volume + 0.05)  # Máximo 1.0
    set_volume(volume)

def DownVolume():
    global volume
    volume = max(0.0, volume - 0.05)  # Mínimo 0.0
    set_volume(volume)
...
def menu_seleccion_modo():
    ...
    while ejecutando:
        ...
        texto_volumen = fuente_pequena.render(f"Volumen: {get_volume_percentage()}% (+/- para ajustar, M para silenciar)", True, (255, 255, 255))
        ventana.blit(texto_volumen, (20, ALTO - 30))
...
if __name__ == "__main__":
    import juegoEscapaLaberinto.presaMode as presaMode
    import juegoEscapaLaberinto.cazadorMode as cazadorMode
    import juegoEscapaLaberinto.salonFama as salonFama
    
    pygame.init()
    
    # Iniciar música al arrancar
    try:
        play('music.mp3')
    except Exception as e:
        print(f"No se pudo cargar la música: {e}")
